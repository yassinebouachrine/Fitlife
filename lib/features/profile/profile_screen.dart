...
class _ProfileScreenState extends State<ProfileScreen> {
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        setState(() {
          ...
          _height = _toDouble(user['height']);
          _goal = user['goal']?.toString() ?? '';
    ...
    if (_error.isNotEmpty) {
      return Scaffold(
        body: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(_error, textAlign: TextAlign.center, style: TextStyle(color: cs.onSurface.withOpacity(0.5))),
          const SizedBox(height: 16),
          ElevatedButton.icon(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          Row(children: [
            ...
            _st(cs, Icons.local_fire_department_outlined, '${_streak}d', 'Streak', c.C.amber),
            const SizedBox(width: 8),
            _st(cs, Icons.bolt_outlined, '$_xp', 'XP', c.C.violet),
          ]),
          const SizedBox(height: 22),

          _titleW(cs, 'Achievements'),
          const SizedBox(height: 10),
          if (_achievements.isEmpty)
            AppCard(child: Center(child: Padding(padding: const EdgeInsets.all(8),
              child: Text('No achievements yet. Keep working out!',
                  style: TextStyle(fontSize: 13, color: cs.onSurface.withOpacity(0.4))))))
          else
            SingleChildScrollView(scrollDirection: Axis.horizontal,
              child: Row(children: _achievements.map((a) => Padding(
                padding: const EdgeInsets.only(right: 16),
                child: _badge(cs, Icons.emoji_events_outlined, a['name']?.toString() ?? '', c.C.amber),
              )).toList())),
          const SizedBox(height: 22),

          Row(children: [
            _titleW(cs, 'Fitness profile'),
            const Spacer(),
            TextButton(onPressed: () {}, style: TextButton.styleFrom(foregroundColor: cs.primary,
          ...
          SizedBox(width: double.infinity, height: 48, child: OutlinedButton.icon(
            ...
            style: OutlinedButton.styleFrom(foregroundColor: c.C.rose,
  ...
  Widget _st(ColorScheme cs, IconData icon, String v, String l, Color col) => Expanded(child: AppCard(
    child: Column(children: [
      Container(width: 34, height: 34, decoration: BoxDecoration(color: col.withOpacity(0.06), borderRadius: BorderRadius.circular(9)),
  ...
  Widget _badge(ColorScheme cs, IconData icon, String l, Color col) => Column(children: [
    Container(width: 44, height: 44, decoration: BoxDecoration(color: col.withOpacity(0.06), borderRadius: BorderRadius.circular(12)),
  ...
  Widget _info(ColorScheme cs, IconData icon, String l, String v, Color col) => Padding(
    ...
    child: Row(children: [
      Container(width: 30, height: 30, decoration: BoxDecoration(color: col.withOpacity(0.06), borderRadius: BorderRadius.circular(8)),
  ...
  Widget _row(ColorScheme cs, IconData icon, String l) => Padding(
    padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 13),
    child: Row(children: [
      Container(width: 30, height: 30, decoration: BoxDecoration(color: cs.onSurface.withOpacity(0.04), borderRadius: BorderRadius.circular(8)),
