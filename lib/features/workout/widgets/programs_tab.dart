...
class _ProgramsTabState extends State<ProgramsTab> {
  ...
  Future<void> _loadPrograms() async {
    ...
    try {
      ...
      final response = await _programService.getPrograms(category: category == 'All' ? null : category);
      if (response['success'] == true) {
        setState(() { _programs = List<Map<String, dynamic>>.from(response['data'] ?? []); _loading = false; });
      } else {
  ...
  void _startProgram(Map<String, dynamic> program) {
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => ActiveWorkoutScreen(
        workoutName: program['name']?.toString() ?? 'Program',
        programId: _toInt(program['id']),
      ),
    )).then((result) { if (result == true) _loadPrograms(); });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      onRefresh: _loadPrograms, color: cs.primary,
      child: SingleChildScrollView(
        physics: const AlwaysScrollableScrollPhysics(parent: BouncingScrollPhysics()),
        padding: const EdgeInsets.fromLTRB(20, 16, 20, 32),
        child: Column(children: [
          SizedBox(width: double.infinity, height: 48, child: ElevatedButton.icon(
            onPressed: () {}, icon: const Icon(Icons.auto_awesome_outlined, size: 18),
            ...
            itemBuilder: (_, i) {
              ...
              return GestureDetector(
                ...
                child: Container(padding: const EdgeInsets.symmetric(horizontal: 14),
                  ...
                  child: Center(child: Text(_tags[i], style: TextStyle(fontSize: 13,
          ...
          if (_loading) const Padding(padding: EdgeInsets.all(40), child: CircularProgressIndicator())
            ...
            Padding(padding: const EdgeInsets.all(40), child: Text('No programs found', style: TextStyle(color: cs.onSurface.withOpacity(0.5))))
          else
            ..._programs.asMap().entries.map((entry) {
              final i = entry.key;
              final p = entry.value;
              final iconsList = [Icons.fitness_center_outlined, Icons.local_fire_department_outlined, Icons.directions_run_outlined, Icons.self_improvement_outlined];
              final colorsList = [c.C.blue600, c.C.rose, c.C.emerald, c.C.cyan];
              return Padding(
                padding: EdgeInsets.only(bottom: i < _programs.length - 1 ? 10 : 0),
                child: _card(cs, p, iconsList[i % iconsList.length], colorsList[i % colorsList.length]),
              );
            }),
        ]),
      ),
  ...
  Widget _card(ColorScheme cs, Map<String, dynamic> p, IconData icon, Color col) => AppCard(
    padding: EdgeInsets.zero,
    child: Column(children: [
      Padding(padding: const EdgeInsets.fromLTRB(14, 14, 14, 10), child: Row(children: [
        Container(width: 40, height: 40, decoration: BoxDecoration(color: col.withOpacity(0.08), borderRadius: BorderRadius.circular(10)),
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(p['description']?.toString() ?? '', style: TextStyle(fontSize: 12, color: cs.onSurface.withOpacity(0.38)), maxLines: 1, overflow: TextOverflow.ellipsis),
        ])),
      ...
      Padding(padding: const EdgeInsets.fromLTRB(14, 10, 14, 12), child: Row(children: [
        _tag(cs, Icons.schedule_outlined, '${_toInt(p['session_duration_min'])} min'),
        const SizedBox(width: 12),
        _tag(cs, Icons.date_range_outlined, '${_toInt(p['duration_weeks'])} wk'),
        const SizedBox(width: 12),
        _tag(cs, Icons.bar_chart_outlined, p['difficulty']?.toString() ?? ''),
        const Spacer(),
        SizedBox(height: 32, child: ElevatedButton(
          onPressed: () => _startProgram(p),
